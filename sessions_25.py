...
# %%
df = pd.concat([df, pd.DataFrame([theo])])
# %%
list_of_players = [main.Player(df.loc[name]) for name in df.index]
# %%
session_of_rounds = main.SessionOfRounds(
    list_of_players,
    amount_of_rounds=4,
    preferences=["balanced"] * 2 + ["level"] * 2,
    level_gap_tol=1,
    num_iter=40,
    seed=1,
)
...
# %%
df = pd.concat([df, pd.DataFrame([stella, patrick, louise])])
# %%
list_of_players = [main.Player(df.loc[name]) for name in df.index]
# %%
session_of_rounds_2 = main.SessionOfRounds(
...
df = pd.concat([df, pd.DataFrame([Marius, antoine])])
# %%
reload(main)
list_of_players = [main.Player(df.loc[name]) for name in df.index]
...
df = pd.concat([df, pd.DataFrame([leila, plus_1_enzo, plus_1_felix])])
df["Happiness"] = 0
# %%
reload(main)
...
df = pd.concat([df, pd.DataFrame([leila, florian, plus_1_angela, marco])])
df["Happiness"] = 0
# %%
reload(main)
...
# %%
df = pd.concat([df, pd.DataFrame([Elena])])
df["Happiness"] = 0
# %%
reload(main)
first_seed = 3
